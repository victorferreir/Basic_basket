    ...
    if 1 <= optionUser <= 2:
        if optionUser == 1:
            ...
            if 1 <= chosenProduct <= 18:
                productQuantity = int(
                    input(
                        "\nVocê escolheu o produto " + products[chosenProduct - 1] + ". Insira a quantidade desejada: "))
                ...
                print("\n{} quantidade(s) de {} é R$ {:.2f}".format(productQuantity, products[chosenProduct - 1],valueSumQuantityProduct))
            else:
                print("\nINSIRA UM CÓDIGO VÁLIDO\n")

        else:
            for counter in range(len(listChosenProducts)):
                print("{}: {} {} - R$ {:.2f}".format(counter + 1, str(productQuantity), listChosenProducts[counter],listValuesChosenProducts[counter]))
